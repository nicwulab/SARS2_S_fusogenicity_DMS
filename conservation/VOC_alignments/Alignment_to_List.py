...
for record in SeqIO.parse('SARS_CoV2_VOC_VOI_Blast_results_alignment.txt', 'fasta'):
...
for res in range(len(query_seq)):
    if query_seq[res] != '-':
        ...
        freq_result_sub.append(count/15)
...
data_frame.to_csv("SARS_CoV2_S_S2_sequence_conservation.csv", index = False, header = ["pos","resi","conservation_count","align_freq"])